...
const CompositionDonut: React.FC<{ segments: Seg[]; centerLabel: string; centerSub?: string }> = ({ segments, centerLabel, centerSub }) => {
    ...
    total > 0
      ? segments
          ...
          .map((s) => {
            ...
            const el = <circle key={s.name} cx='21' cy='21' r='15.9155' fill='none' stroke={s.color} strokeWidth='6' strokeDasharray={`${len.toFixed(3)} ${(100 - len).toFixed(3)}`} strokeDashoffset={offset.toFixed(3)} transform='rotate(-90 21 21)' />;
            offset -= len;
            return el;
          })
      : [];
